...
import com.example.memehub.ui.components.StateSelector
...
fun SignUpScreenContent(
    ...
    onCountryChange: (CountriesQuery.Country) -> Unit,
    onStateChange: (CountriesQuery.State) -> Unit,
    onSignUpClick: () -> Unit
) {
    ...
    Scaffold(
        ...
        Column(
            ...
            var selectedStatedIndex by remember {
                mutableIntStateOf(-1)
            }
            ...
            StateSelector(
                label = "State",
                items = uiState.country.states,
                selectedIndex = selectedStatedIndex,
                onItemSelected = { index, item -> run {
                    selectedStatedIndex = index
                    onStateChange(item)
                } })
